    ...
    <>
      ...
      <section className="relative h-screen w-full overflow-hidden">
        ...
        <div className="relative z-10 flex h-full items-center justify-center px-6">
          <div className="text-center">
            ...
            <div className="mx-auto mt-10 h-px w-64 bg-blue/70" />
      ...
      <section className="py-32 bg-gray-50">
        <div className="max-w-7xl mx-auto px-8">
          ...
          <div className="mb-32">
            <div className="grid md:grid-cols-2 gap-16 items-center">
              <div className="order-2 md:order-1">
                ...
                <span className="inline-block px-6 py-3 bg-blue text-white text-lg font-medium uppercase tracking-wider rounded-full">
              ...
              <div className="order-1 md:order-2 relative">
                ...
                <button onClick={() => prevSlide(setBudanIndex, budanilkanthaMedia, budanIndex)} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/60 text-white p-4 rounded-full hover:bg-blue transition text-2xl">
                  ‹
                </button>
                <button onClick={() => nextSlide(setBudanIndex, budanilkanthaMedia, budanIndex)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/60 text-white p-4 rounded-full hover:bg-blue transition text-2xl">
                  ›
                </button>

                {isVideo(budanilkanthaMedia[budanIndex]) && (
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-4 bg-black/70 px-6 py-3 rounded-full">
                    <button onClick={() => togglePlayPause(budanVideoRef)} className="text-white hover:text-blue-light transition text-xl">
                      {budanVideoRef.current?.paused !== false ? '▶' : '❚❚'}
                    </button>
                    <button onClick={() => goFullscreen(budanVideoRef)} className="text-white hover:text-blue-light transition text-xl">
                      ⛶
                    </button>
                  </div>
                )}
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                  {budanilkanthaMedia.map((_, i) => (
                    <div key={i} className={`w-3 h-3 rounded-full transition ${i === budanIndex ? 'bg-blue' : 'bg-white/50'}`} />
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="mb-32">
            <div className="grid md:grid-cols-2 gap-16 items-center">
              <div className="relative">
                ...
                <button onClick={() => prevSlide(setSunakothiIndex, sunakothiMedia, sunakothiIndex)} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/60 text-white p-4 rounded-full hover:bg-blue transition text-2xl">
                  ‹
                </button>
                <button onClick={() => nextSlide(setSunakothiIndex, sunakothiMedia, sunakothiIndex)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/60 text-white p-4 rounded-full hover:bg-blue transition text-2xl">
                  ›
                </button>

                {isVideo(sunakothiMedia[sunakothiIndex]) && (
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-4 bg-black/70 px-6 py-3 rounded-full">
                    <button onClick={() => togglePlayPause(sunakothiVideoRef)} className="text-white hover:text-blue-light transition text-xl">
                      {sunakothiVideoRef.current?.paused !== false ? '▶' : '❚❚'}
                    </button>
                    <button onClick={() => goFullscreen(sunakothiVideoRef)} className="text-white hover:text-blue-light transition text-xl">
                      ⛶
                    </button>
                  </div>
                )}
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                  {sunakothiMedia.map((_, i) => (
                    <div key={i} className={`w-3 h-3 rounded-full transition ${i === sunakothiIndex ? 'bg-blue' : 'bg-white/50'}`} />
                  ))}
                </div>
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-16 items-center">
              ...
              <div className="order-1 md:order-2 relative">
                ...
                {balkumariMedia.length > 1 && (
                  <>
                    <button onClick={() => prevSlide(setBalkumariIndex, balkumariMedia, balkumariIndex)} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/60 text-white p-4 rounded-full hover:bg-blue transition text-2xl">
                      ‹
                    </button>
                    <button onClick={() => nextSlide(setBalkumariIndex, balkumariMedia, balkumariIndex)} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/60 text-white p-4 rounded-full hover:bg-blue transition text-2xl">
                      ›
                    </button>

                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                      {balkumariMedia.map((_, i) => (
                        <div key={i} className={`w-3 h-3 rounded-full transition ${i === balkumariIndex ? 'bg-blue' : 'bg-white/50'}`} />
                      ))}
                    </div>
                  </>
                )}
          ...
          {/* CTA */}
          <div className="text-center">
            <button onClick={goToContactTop} className="inline-block bg-blue text-white px-16 py-6 text-2xl uppercase tracking-widest font-medium rounded-full hover:bg-blue-dark transition duration-300 shadow-2xl">
